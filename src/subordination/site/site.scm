  ...
  (let ((tag-files (assoc-ref doxyfile 'tagfiles)))
    (if tag-files
        (map
         ...
         (filter (lambda (lst) (<= 2 (length lst)))
                 (map (lambda (value) (string-split value #\=))
                      (list tag-files))))
        '())))
           ...
           (filter
            (lambda (file)
              (and (string? file)
                   ...
                   (not (member file (map car tag-files)))))
            (hash-map->list
     ...
     (let ((old-file (location-file (definition-location value))))
       (set! (definition-output-path value)
             (cond
              ...
              ((and
                 (string? old-file)
                 (not (string-null? old-file))
                 (not (member old-file (map car tag-files))))
  ...
  (hash-for-each
   (lambda (key value)
     (if (and (string= "dir" (definition-kind value))
              (string-suffix? "/src" (definition-name value)))
         (hash-remove! definitions key)))
   definitions))
