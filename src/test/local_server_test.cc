...
TYPED_TEST(LocalServerTest, DetermineIdRange) {
	...
	id_type id0 = 0;
	id_type id1 = 0;
	bsc::determine_id_range(ifaddr, id0, id1);
}

template <class Addr, class Id>
void
test_id_count(const Id& expected, const sys::ifaddr<Addr>& ifaddr) {
	typedef Id id_type;
	id_type id0 = 0;
	id_type id1 = 0;
	bsc::determine_id_range(ifaddr, id0, id1);
...
TEST(LocalServer, IntersectIpV4) {
	...
	typedef sys::ifaddr<addr_type> ifaddr_type;
	typedef typename addr_type::rep_type id_type;
	ifaddr_type ifaddr{{127,0,0,1}, 24};
	const id_type cnt = ifaddr.count();
	id_type id0 = 1;
	id_type id1 = 1;
	id_type id2 = 0;
	id_type id3 = 0;
