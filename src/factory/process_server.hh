	...
	struct Process_rserver: public Buffered_server<T,Router> {
		...
		send(kernel_type* kernel) {
			...
			*/
			...
			_ostream.end_packet();
			#ifndef NDEBUG
			stdx::debug_message("app", "send")
				<< stdx::make_object("to", _childpid, "kernel", *kernel);
			#endif
...
//				}
		...
		forward(const Kernel_header& hdr, sys::packetstream& istr) {
			...
			_ostream.end_packet();
			#ifndef NDEBUG
			stdx::debug_message("app", "forward")
				<< stdx::make_object("header", hdr);
			#endif
		}
	...
	private:
		...
		on_process_exit(sys::process& p, sys::proc_info status) {
			...
			if (result != this->_apps.end()) {
				#ifndef NDEBUG
				stdx::debug_message("app", "exit")
					<< stdx::make_object("status", status, "app", result->first);
				#endif
				_apps.erase(result);
			}
		}
